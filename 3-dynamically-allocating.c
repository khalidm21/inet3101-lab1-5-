...
int main(int argc, char *argv[]) {
    ...
    //the updated code takes the command line input string and converts it to an integer
    //it then initalizes the variable to keep track of the allocated memory initlially set to the users input
    //lastly we use the malloc function to allocate the memory dynamically
    int num_elements = atoi(argv[1]);
    int current_size = num_elements; // Added to track size
    int *array = (int *)malloc(current_size * sizeof(int));

    // updated code for infinte loop - now the user enters the data, type in a number, hit return/enter, enter another until done.
    printf("Keep entering numbers. This will run indefinitely.\n");
    int input, index = 0;
    while (1) { // changed to infinite loop
        if (index == current_size) { // checking tp see if the array full
            current_size += 5; // if it is full resiize the  by 5 spaces
            array = (int *)realloc(array, current_size * sizeof(int)); // this is where the memory is resized dynamically
        }  
     
    printf("Enter number: ");   
    scanf("%d", &input); 
   
    array[index++] = input; // inputting infinitely
    }

    // then print out what was entered.
    printf("Numbers in the array:\n");
    for (int i = 0; i < index; i++) { // looping to display the last input
        printf("%d ", array[i]);
    }
    printf("\n");
